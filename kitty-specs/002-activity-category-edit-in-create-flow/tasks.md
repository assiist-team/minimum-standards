...
## Work Package WP01: Add Category Picker to ActivityModal (Priority: P1) 🎯 MVP
...
**Independent Test**: Open ActivityModal in create mode → verify Category field appears between Unit and Notes → select a category → save → verify the activity is created with the selected categoryId. Repeat in edit mode to verify pre-population.
...
- The save handler at line ~122 currently reads `categoryId: activity?.categoryId ?? null` — change this to read from the new `categoryId` state variable.
...
### Risks & Mitigations
- **Risk**: `useCategories()` returns empty on first render → Ensure the picker row shows "None" gracefully when categories are loading.
- **Risk**: Category picker z-index conflicts with modal → `BottomSheetMenu` already handles overlay correctly as a nested bottom sheet.
...
**Independent Test**: Select an activity on Step 1 → verify Edit button appears → tap Edit → verify ActivityModal opens in edit mode with Category field → change category → save → verify updated category displays on Step 1. Also verify no category picker overlay appears anywhere.
...
- **Convert** the category `TouchableOpacity` row (lines 234-242) to a plain `View` — remove `onPress` and the edit `MaterialIcons` icon.
...
### Risks & Mitigations
- **Risk**: Editing the selected activity could cause stale data in `standardsBuilderStore` → Mitigated by calling `setSelectedActivity()` with updated data in the save callback.
- **Risk**: Two `ActivityModal` instances (create + edit) mounted simultaneously → Only one is visible at a time (`showActivityModal` vs `editModalVisible`), so no conflict.
